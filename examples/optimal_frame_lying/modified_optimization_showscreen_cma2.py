...
def cma_ws_optimizer(frame_i,env):
    ...
    #x0 = np.array([-0.1, -0.1, 0 ])
    points_old = np.array(loaded_dict['Points'])
    xp = points_old[frame_i] - np.array([0,0,0.015])
    ...
    optimizer = CMA(mean=np.array(x0), sigma=0.1, population_size=pop_size)
    fevals = 0
    iteration = 0
    t0 = time.time()
...
if __name__ == "__main__":
    ...
    f = open("file_modified_showscreen_50_second_cma2.pkl","wb")
