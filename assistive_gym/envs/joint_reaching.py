...
class JointReachingEnv(AssistiveEnv):
    ...
    def _get_obs(self, agent=None):
        self.create_sphere(radius=0.04, mass=0.0, pos=[-0.4,-0.25,1.0], visual=True, collision=False, rgba=[1, 0, 0, 1])
        p.stepSimulation()
        time.sleep(10)
    ...
    def reset(self):
        ...
        tool_pos,tool_orient = self.robot.get_pos_orient(self.robot.left_end_effector)
        head_t_pos,head_t_orient = self.human.get_pos_orient(self.human.base)
        head_t_pos_new = [head_t_pos[0],head_t_pos[1],head_t_pos[2]+0.2]
        self.generate_line(head_t_pos_new,head_t_orient)
        self.generate_line([-0.4,-0.25,1.0],tool_orient,0.3)
