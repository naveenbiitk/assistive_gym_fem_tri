        ...
        for h_j_ in range(-3,-1,2):  #head_z
            for j_ in range(-3,4,2):   # waist orientation
                for i_ in range(-3,4,2):  # base orientation
                    ...
                    evaluation_orient = orient_wrt_body_frames(orient, env, frame_i)
...
def scipy_optimizer(frame_i, env):
    ...
    xp = points_old[frame_i] - np.array([0,0,0.15])
    ...
    bnd = ((bnd_point[0]-1.5*arm_length,bnd_point[0]+0.5*arm_length),(bnd_point[1]- 1.5*arm_length,bnd_point[1]- 0.5*arm_length),(bnd_point[2]+0.7*arm_length,bnd_point[2]+1.2*arm_length),(-np.pi/2,np.pi/2),(-np.pi/2,np.pi/2),(-np.pi/2,np.pi/2))
...
if __name__ == "__main__":
    ...
    f = open("file_modified_lying_showscreen_orient_eye_second.pkl","wb")
