...
class HumanRestingEnv(AssistiveEnv):
    ...
    def reset(self):
        ...
        #self.human.body_shape = torch.from_numpy(np.array(data1['betas'])) #torch.Tensor(data1['betas'])
        
        self.build_assistive_env(furniture_type='hospital_bed', fixed_human_base=False, gender='male', human_impairment='none')
        ...
        p.setGravity(0, 0, -9.815, physicsClientId=self.id)
        self.human.set_gravity(0, 0, -9.815)
    ...
    def load_smpl_model(self):
        ...
        print(self.f_name, data1)
        #df = torch.Tensor(data1['body_pose'])
        df = torch.from_numpy(np.array(data1['body_pose']))
        dt = torch.reshape(df, (1, 24, 3))
        db = dt[:,1:22,:]
        
        orient_tensor = torch.from_numpy(np.array(data1['global_orient'])) #torch.Tensor(data1['global_orient'])
