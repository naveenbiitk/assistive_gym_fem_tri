...
# human_base_pose, oreint = env.human.get_pos_orient(env.human.base)
# quaternion to euler () 
# new_h_orient = euler to quaternion ()
# env.human.set_base_pos_orient(human_base_pose, new_h_orient )


for i_ in range(-5,5,2):
    for j_ in range(-5,5,2):
        for k_ in range(-5,5,2):
            i = i_*5
            j = j_*5
            k = k_*5
            joints_positions = [(env.human.j_waist_x, i), (env.human.j_waist_y, j), (env.human.j_waist_z, k)]
            env.human.setup_joints(joints_positions, use_static_joints=True, reactive_force=None)
            run_simulation(env.human, env.id, 15)        
            print('Simulation'+str(i_)+ 'done-----------')