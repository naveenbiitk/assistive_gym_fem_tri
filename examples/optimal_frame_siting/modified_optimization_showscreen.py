...
def orient_wrt_body_frames(orient, env, frame_i):

    head_point = np.array(p.getEulerFromQuaternion(env.human.get_pos_orient(env.human.head)[1]),dtype="float64")
    neck_point = np.array(p.getEulerFromQuaternion(env.human.get_pos_orient(env.human.neck)[1]),dtype="float64")
    chest_point = np.array(p.getEulerFromQuaternion(env.human.get_pos_orient(env.human.right_pecs)[1]),dtype="float64")
    waist_point = np.array(p.getEulerFromQuaternion(env.human.get_pos_orient(env.human.waist)[1]),dtype="float64")
    stomach_point = np.array(p.getEulerFromQuaternion(env.human.get_pos_orient(env.human.stomach)[1]),dtype="float64")
    left_shoulder_point = np.array( p.getEulerFromQuaternion(env.human.get_pos_orient(env.human.left_shoulder)[1]),dtype="float64")
    right_shoulder_point = np.array(p.getEulerFromQuaternion(env.human.get_pos_orient(env.human.right_shoulder)[1]),dtype="float64")
    left_elbow_point = np.array( p.getEulerFromQuaternion(env.human.get_pos_orient(env.human.left_elbow)[1]),dtype="float64")
    right_elbow_point = np.array(p.getEulerFromQuaternion(env.human.get_pos_orient(env.human.right_elbow)[1]),dtype="float64")

    target_points = np.zeros([9,3])

    target_points[0] = head_point + orient
    target_points[1] = neck_point + orient
    target_points[2] = chest_point + orient
    target_points[3] = waist_point + orient
    target_points[4] = stomach_point + orient
    target_points[5] = left_shoulder_point + orient
    target_points[6] = right_shoulder_point + orient
    target_points[7] = left_elbow_point + orient
    target_points[8] = right_elbow_point + orient

    return target_points[frame_i]



def optimizing_across_poses(point_full, env, frame_i):
        ...
        for h_j_ in range(-3,-1,2):  #head_z
            for j_ in range(-3,4,2):   # waist orientation
                for i_ in range(-3,4,2):  # base orientation
                    ...
                    evaluation_orient = point_wrt_body_frames(orient, env, frame_i)
                    #print('Evaluation point: ',evaluation_point)
                    score_array[index] = cmaes_showphone.optimizing_function(evaluation_point,evaluation_orient, env.human, env.id, env) #evaluate_object_handover(evaluation_point)
                    index=index+1
...
def show_bounds(env, arm_length, bnd_point, frame_i):
    ...
    x_min = bnd_point[0]+head_point[0] - arm_length 
    x_max = bnd_point[0]+head_point[0] + arm_length

    y_min = bnd_point[1]+head_point[1] - 2.5*arm_length
    y_max = bnd_point[1]+head_point[1] - 1.5*arm_length
...
def bound_offset(env, frame_i):
    ...
    target_points[0] = head_point - head_point 
    target_points[1] = head_point - neck_point 
    target_points[2] = head_point - chest_point
    target_points[3] = head_point - waist_point 
    target_points[4] = head_point - stomach_point 
    target_points[5] = head_point - left_shoulder_point 
    target_points[6] = head_point - right_shoulder_point 
    target_points[7] = head_point - left_elbow_point 
    target_points[8] = head_point - right_elbow_point 
...
def scipy_optimizer(frame_i, env):
    ...
    xp = points_old[frame_i] - np.array([0,0,0])

    x0 = np.array([xp[0], xp[1], xp[2], 0, -0.35, 0])
    bnd_point = bound_offset(env, frame_i)
    #bnd = ((bnd_point[0]-arm_length,bnd_point[0]+arm_length),(bnd_point[1]- 2.5*arm_length,bnd_point[1]- 1.5*arm_length),(bnd_point[2]-arm_length,bnd_point[2]+arm_length))

    bnd_orient = ((-np.pi/2,np.pi/2),(-np.pi/2,np.pi/2),(-np.pi/2,np.pi/2))

    bnd = ((bnd_point[0]-arm_length,bnd_point[0]+arm_length),(bnd_point[1]- 2.5*arm_length,bnd_point[1]- 1.5*arm_length),(bnd_point[2]-arm_length,bnd_point[2]+arm_length),(-np.pi/2,np.pi/2),(-np.pi/2,np.pi/2),(-np.pi/2,np.pi/2))

    #show_bounds(env, arm_length, bnd_point, frame_i)
    #time.sleep(1)
    print('Initial point: ',x0)
    print('Bounds ',bnd) 
    opt = {'disp':True,'maxiter':25,'maxfev':25} 
...
def cma_optimizer(frame_i):
    ...
    x0 = np.array([-0.1, -0.1, 0 ])
    bnd = ((-arm_length,arm_length),(- 2.5*arm_length,- 1.5*arm_length),(-arm_length,arm_length))
    ...
    es.optimize(optimizing_across_poses,args=(env, frame_i), opts=opts, callback=es.logger.plot )
    es.result_pretty()
    cma.plot()
    logger.plot()
    print('FInished')
...
if __name__ == "__main__":
    ...
    f = open("file_modified_showscreen_orient_eye.pkl","wb")
