...
def neck_angle(env_human, env_id, target_pos):
    ...
    p0 = eye_pos
    p1 = generate_line_head(eye_pos, heye_orient)
    p2 = target_pos
...
def run_simulation(env_human, env_id, target_pose_angles, count_target):
    coop = True
    count_=0
    t=0
...
def optimizing_function(points, env_human, env_id, print_flag=False):
    ...
    weights_opt = np.array([ 1,  1,  0.1 , 1])
    dist_threshold=0.1
    ...
    run_simulation(env_human, env_id, target_pose_angles , 15)
    env_human.set_joint_angles(env_human.right_arm_joints, target_pose_angles)
...
optimizing_function(points, env.human, env.id, print_flag=True)
