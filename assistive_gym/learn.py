...
def setup_config(env, algo, coop=False, seed=0, extra_configs={}):
    ...
    if algo == 'ppo_hf':
        ...
        config['model']['fcnet_hiddens'] = [100, 100]
        config['lr'] = 5e-5 # default = 5e-5
        config['callbacks'] = CustomRewardCallback
...
# def train_hf(env_name, algo, timesteps_total=1000000, save_dir='./trained_models/', load_policy_path='', coop=False, seed=0, extra_configs={}):
#     ray.init(num_cpus=multiprocessing.cpu_count(), ignore_reinit_error=True, log_to_driver=False)
#     env = make_env(env_name, coop)
#     agent, checkpoint_path = load_policy(env, algo, env_name, load_policy_path, coop, seed, extra_configs)
#     env.disconnect()
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            ...
            if coop:
                ...
                #import numpy as np
                action_human[np.isnan(action_human)] = 0.0
                action_robot[np.isnan(action_robot)] = 0
                action_robot[2]=0
                obs, reward, done, info = env.step({'robot': action_robot*0.17, 'human': action_human})
            ...
            if colab:
                ...
                frames.append(img)
        vid.close()
        print('---Video saved to', filename_vid)
            # save the action and observation in .npz file
        np.savez(filename_npz, action_list=action_list, observation_list=observation_list)
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            if coop:
                ...
                action_human = test_agent.compute_action(obs['human'], policy_id='human')
                # Step the simulation forward using the actions from our trained policies
                obs, reward, done, info = env.step({'robot': action_robot*0.1, 'human': action_human})
                reward = reward['robot']
                done = done['__all__']
                info = info['robot']
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_hf', action='store_true', default=False,
                        help='Whether to train a new policy from human feedback')
    ...
    # if args.train_hf:
    #     checkpoint_path = train_hf(args.env, args.algo, timesteps_total=args.train_timesteps, save_dir=args.save_dir, load_policy_path=args.load_policy_path, coop=coop, seed=args.seed)
    if args.render:
