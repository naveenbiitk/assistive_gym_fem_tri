...
class AssistiveEnv(gym.Env):
    def __init__(self, robot=None, human=None, task='', obs_robot_len=0, obs_human_len=0, time_step=0.02, frame_skip=5, render=False, gravity=-9.81, seed=1001):
        ...
        #print('--env obs_human_len---', obs_human_len)
        ...
        print('--env obs_human_len---', self.observation_space)
    ...
    def take_step(self, actions, gains=None, forces=None, action_multiplier=0.05, step_sim=True):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if needs_action:
                ...
                if isinstance(agent, Robot):
                    if agent.gripper_included:
                        action_name = int(actions[2]>0)   #no wheel else 4
            ...
            for _ in range(self.frame_skip):#frame_skip is 5
                ...
                if isinstance(agent, Human) and agent.impairment == 'tremor':
                    ...
                    if isinstance(agent, Robot) and agent.gripper_included:
                        agent_joint_angles[2]=0.01   #no wheel else 4
            ...
            if isinstance(agent, Robot) and agent.action_duplication is not None:
                ...
                if agent.gripper_included:
                    agent.perform_special_gripper_action(action_name, agent.left_gripper_indices)   #action_name == 1: #1 closed
    ...
    def setup_camera(self, camera_eye=[-0.5, -0.75, 1.5], camera_target=[-0.2, 0, 1.25], fov=60, camera_width=1920//2, camera_height=1080//2):
    ...
    def setup_camera_rpy(self, camera_target=[-0.2, 0, 0.75], distance=1.5, rpy=[0, -35, 40], fov=60, camera_width=1920//2, camera_height=1080//2):
        self.camera_width = camera_width
        self.camera_height = camera_height
