...
# for i_ in range(len(det_list)):
#     env.create_sphere(radius=0.01, mass=0.0, pos= det_list_points[i_])
#print('Det list',len(det_list))

with open('file_handover_combine.pkl', 'rb') as f:
...
combine_points = np.array(loaded_dict['combine'])

# human_height=1.1

# for i_ in range(len(combine_points)):
#     if combine_points[i_][2]<human_height:
#         env.create_sphere(radius=0.01, mass=0.0, pos= combine_points[i_], visual=True, collision=False, rgba=[1, 1, 0, 1]) 



# for sim_ in range(len(combine_points)):
#     target_position = combine_points[i_]
...
#     linear_moment = np.sqrt(forces[:,3]*forces[:,3] + forces[:,4]*forces[:,4] + forces[:,5]*forces[:,5] )
    ...
    # print('Forces', np.linalg.norm(linear_forces),np.linalg.norm(linear_moment) )
    # if jlwki > best_jlwki:
...
env.create_sphere(radius=0.01, mass=0.0, pos= target_pos, visual=True, collision=False, rgba=[1, 1, 0, 1]) 
