...
from .naveenb_utils.coordinate_frame import human_pose_smpl_format, generate_line_hand

class ShowPhoneEnv(AssistiveEnv):

    def __init__(self, robot, human):
        super(ShowPhoneEnv, self).__init__(robot=robot, human=human, task='show_phone', obs_robot_len=16, obs_human_len=0 )
    ...
    def step(self, action):
        ...
        #self.generate_line(wrt_pos,wrt_orient,0.3)



        #joints_positions = [(self.human.j_head_z, self.iteration), (self.human.j_head_x, 0)]
        ...
        if self.gui :
            print('Task success:', self.task_success, 'Tool force at target:', reward_force_scratch)
    ...
    def generate_line(self, pos, orient, lineLen=0.5):
        ...
        p.addUserDebugLine(pos, toZ, [1, 0, 0], 5)
    ...
    def reset(self):
        ...
        self.human.set_base_pos_orient([-0.15, 0.2, 1.00], [-np.pi/2.0, 1.2, 0])
        #self.human.set_base_pos_orient([-0.15, 0.2, 1.50], [-np.pi/2.0, 0, 0])
        ...
        #joints_positions = [(self.human.j_right_shoulder_x, 150),(self.human.j_right_shoulder_z, -90), (self.human.j_right_shoulder_y, -100), (self.human.j_right_elbow, -100)]
        #self.human.setup_joints(joints_positions, use_static_joints=False, reactive_force=0.01)
        ...
        #self.tool.init(self.robot, self.task, self.directory, self.id, self.np_random, right=True, mesh_scale=[0.045]*3, alpha=1.0)
        ...
        self.robot.set_gravity(0, 0, -10)
        #self.tool.set_gravity(0, 0, 0)

        human_pt_position, human_pt_orientation = human_pose_smpl_format(self.human)
