...
class HumanMesh(Agent):
    ...
    def create_smplx_body(self, directory, id, np_random, gender='female', height=None, body_shape=None, joint_angles=[], position=[0, 0, 0], orientation=[0, 0, 0], body_pose=None, left_hand_pose=None, right_hand_pose=None):
        ...
        #output = model(betas=torch.Tensor(data1['betas']), body_pose=torch.Tensor(body_pose), left_hand_pose=left_hand_pose, right_hand_pose=right_hand_pose, return_verts=True)        
        output = model(betas=betas, body_pose=torch.Tensor(body_pose), left_hand_pose=left_hand_pose, right_hand_pose=right_hand_pose, return_verts=True)        
        #print('manual',torch.Tensor(body_pose))
        #print('online',db)
        vertices = output.vertices.detach().cpu().numpy().squeeze()
        ...
        #joints = joints.dot(R.from_euler('y', 180, degrees=True).as_matrix())
        #joints = joints.dot(R.from_euler('z', 180, degrees=True).as_matrix())
        # joints += offset
