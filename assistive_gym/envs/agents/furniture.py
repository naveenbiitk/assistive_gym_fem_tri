...
class Furniture(Agent):
    ...
    def init(self, furniture_type, directory, id, np_random, wheelchair_mounted=False):
        if 'wheelchair' in furniture_type:
            ...
            furniture = p.loadURDF(os.path.join(directory, 'bed', 'hospital_bed.urdf'), basePosition=[-0.1, 0, 0.43], baseOrientation=[0, 0, 0, 1],flags=p.URDF_USE_SELF_COLLISION, physicsClientId=id)
            self.controllable_joint_indices = [1]
            super(Furniture, self).init(furniture, id, np_random)
        elif furniture_type == 'table':
            furniture = p.loadURDF(os.path.join(directory, 'table', 'table_tall.urdf'), basePosition=[0.25, -1.0, 0], baseOrientation=[0, 0, 0, 1],flags=p.URDF_USE_SELF_COLLISION, physicsClientId=id)
        elif furniture_type == 'chair':
            furniture = p.loadURDF(os.path.join(directory, 'chair', 'chair.urdf'), basePosition=[-0.30, 0, -1.5], baseOrientation=[0, 0, 0, 1], physicsClientId=id)
