...
class Stretch(Robot):
    def __init__(self, controllable_joints='right'):
        ...
        right_arm_joint_indices = [3, 5, 9]  #3 lift  9 is yaw
        ...
        tool_pos_offset = {'scratch_itch': [0, 0, 0], # Position offset between tool and robot tool joint
                           ...
                           'joint_motion': [0, 0, 0],
                           'robot_motion': [0, 0, 0], 
                           'stretch_testing': [0.11, 0, -0.07],
                           'arm_manipulation': [0.11, 0, -0.07],
                           'joint_reaching': [0, 0, 0.18]}
        tool_orient_offset = {'scratch_itch': [0, 0, 0], # RPY orientation offset between tool and robot tool joint
                              ...
                              'stretch_testing': [0, 0, 0],
                              'object_handover': [0, 0, 0],
                              'show_phone': [0, 0, 0],
                              'joint_motion': [0, 0, 0],
                              'robot_motion': [np.pi/2, np.pi/2, 0],
        ...
        toc_base_pos_offset = {'scratch_itch': [-1.0, -0.1, 0.09], # Robot base offset before TOC base pose optimization
                               ...
                               'show_phone': [-0.85, -0.1, 0.09],
                               'joint_motion': [-1.85, -0.1, 0.09],
                               'robot_motion': [-1.85, -0.1, 0.09],
        ...
        toc_ee_orient_rpy = {'scratch_itch': [0, 0, np.pi/2.0], # Initial end effector orientation
                             ...
                             'stretch_testing': [0, 0, -np.pi/2.0],
                             'object_handover': [0, 0, np.pi/2.0],
                             'show_phone': [0, 0, np.pi/2.0], 
                             'joint_motion': [0, 0, np.pi/2.0],
                             'robot_motion': [0, 0, np.pi/2.0], 
        ...
        self.gains = [0.1]*2 + [0.01] + [0.025]*5 if 'wheel' in controllable_joints else [0.01] + [0.025]*5  
        self.forces = [10]*2 + [20] + [10]*5 if 'wheel' in controllable_joints else [20] + [10]*5 
        ...
        self.action_multiplier = [3, 0, 0, 1, 0] #joint motion environment uncomment
        # Adjust the speed of each motor by a multiplier of the default speed
        # self.action_multiplier = [3, 3, 2, 1, 2] if 'wheel' in controllable_joints else [2, 1, 2]
        self.action_multiplier = [1, 1, 0.2, 0.2, 0.2] if 'wheel' in controllable_joints else [0.2, 0.2, 0.12]  
        ...
        #self.controllable_joint_indices = [0, 1, 3, 5, 9]  # joint motion environment uncomment
        ...
        super(Stretch, self).__init__(controllable_joints, right_arm_joint_indices, left_arm_joint_indices, wheel_joint_indices, right_end_effector, left_end_effector, right_gripper_indices, left_gripper_indices, gripper_pos, right_tool_joint, left_tool_joint, tool_pos_offset, tool_orient_offset, right_gripper_collision_indices, left_gripper_collision_indices, toc_base_pos_offset, toc_ee_orient_rpy, wheelchair_mounted, half_range=False, controllable_joint_indices=None, action_duplication=self.action_duplication, action_multiplier=self.action_multiplier, flags='stretch')

    def randomize_init_joint_angles(self, task, offset=0):
        if task in ['bed_bathing', 'dressing','object_handover','stretch_testing']:
            self.set_joint_angles([3], [0.99+self.np_random.uniform(-0.01, 0.1)])
        else:
