...
def generate_line(pos, orient, lineLen=0.15):
...
def neck_angle(env_human, env_id, target_pos, disp=False):
    ...
    p0 = heye_pos
    p1 = generate_line_head(heye_pos, heye_orient, 0.7, disp)
    p2 = target_pos
...
def calculate_obj_ik(env, target_pose_angles, target_position):
  ...
  mid_angle = np.array([ 1.28484584e-14,  1.74532925e-01, 1.24455499e-14, -8.98027383e-02, -1.10828408e+00, 6.82478992e-14, -1.11701072e+00, -1.43938821e-14])
...
def position_human_arm(env, target_position, attempts):
    ...
    human_angles = env.human.get_joint_angles(env.human.motion_right_arm_joints)
    iteration = 0
    ...
    while iteration < attempts:
        iteration += 1
        target_joint_angles_h = env.human.ik(env.human.right_wrist, target_position, None, ik_indices=env.human.motion_right_arm_joints, max_iterations=2000)

        env.human.set_joint_angles(env.human.motion_right_arm_joints, target_joint_angles_h)
...
def run_simulation(env_human, env_id, count_target, env, target_position):
    ...
    while count_<count_target:
        ...
        current_joint_angles = env.human.get_joint_angles(env.human.motion_right_arm_joints)
        kp=0.6
        set_p = current_joint_angles + kp*(target_joint_angles_h - current_joint_angles)
        env_human.set_joint_angles(env_human.motion_right_arm_joints, set_p)
        # if coop:
        ...
        theta, is_inside, near_p = neck_angle(env_human, env_id, target_position)

        k1 = -1.2
        k3 = 1.2
        x_yaw = k1*(near_p[0]-target_position[0])
        y_nothing = 0
        z_pitch = k3*(near_p[1]-target_position[1])
...
def optimizing_function(points,orient, env_human, env_id, env):
    
    global best_params, best_costs, fevals, f, dict_file

    mid_angle = np.array([ 1.28484584e-14,  1.74532925e-01, 1.24455499e-14, -8.98027383e-02, -1.10828408e+00, 6.82478992e-14, -1.11701072e+00, -1.43938821e-14])
    prev_angle = np.array([-1.28484584e-14,  1.74532925e-01,  1.24455499e-14, -8.98027383e-02, -1.10828408e+00, 6.82478992e-14, -1.11701072e+00, -1.43938821e-14,  7.85398163e-02,  1.74532925e-01])
    
    # weights_opt = np.array([ 0.5,  0.5,  0.1 ])
    weights_opt = np.array([ 0.7,  0.7,  0.07 , 3, 2])
    ...
    generate_line(target_position, p.getQuaternionFromEuler(orient))


    env_human.set_joint_angles(env_human.motion_right_arm_joints, target_pose_angles_h)
    ...
    prev_angle[0:8]=target_pose_angles_h
    det, jlwki = calculate_human_Jlwki(env_human, env_id,prev_angle ) #should be 10dimension
    det = det/max_det

    #print('--distance--',distance,1/det)
    if distance < dist_threshold:
        det = 1/det
        f_manipulability = det
        #print('------------Reached----------------')
    else:
        det = 0.00000000
        f_manipulability = 10
    ...
    theta_neck, is_inside, _ = neck_angle(env_human, env_id, target_position, True)
    #time.sleep(4)
    if is_inside:
    ...
    #print('----reached--',forces)
    f_moment = np.sum(forces[:,5])/0.5
    f_moment = f_moment #/(max_moment**2)

    f_angle = np.sum( (target_pose_angles_h-mid_angle)**2 )
    f_angle = f_angle/(max_ang**2)

    eye_dist = eye_distance(env_human, target_position, env_id)
    ...
    #print('func moment',f_moment,'func angle',f_angle,'func f_manipulability',f_manipulability,'func neck',f_neck, 'func_eye',f_eye_distance)
    #print('func moment',f_moment,'func angle',f_angle,'func f_manipulability',f_manipulability,'func neck',f_neck, 'func_eye',f_eye_distance)
    f_value = (weights_opt[0]*f_moment + weights_opt[1]*f_angle + weights_opt[2]*f_manipulability + weights_opt[3]*f_neck + weights_opt[4]*f_eye_distance)/(weights_opt[0]+weights_opt[1]+weights_opt[2]+ weights_opt[3] + weights_opt[4] )
