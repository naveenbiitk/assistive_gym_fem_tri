...
env_name = "RobotMotionStretch-v1"
...
robot_actions = {ord('s'): np.array([ 0.01, 0, 0]), ord('x'): np.array([-0.01, 0, 0]), 
                ord('z'): np.array([ 0, -0.01, 0]), ord('c'): np.array([ 0, 0.01, 0]), 
                ord('a'): np.array([ 0, 0, 0.01]), ord('d'): np.array([0, 0, -0.01])}
...
while True:
    ...
    keys = p.getKeyboardEvents()
    #print('---robot-actions_',robot_actions)
