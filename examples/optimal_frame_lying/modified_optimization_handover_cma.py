...
def optimizing_across_poses(point):
...
def cma_optimizer(frame_i, env):
    ...
    opts.set('bounds', [[bnd_point[0]-1.5*arm_length, bnd_point[1]- 1.5*arm_length,bnd_point[2]+0.7*arm_length ],[bnd_point[0]+0.5*arm_length,bnd_point[1]- 0.5*arm_length, bnd_point[2]+1.2*arm_length]])
...
if __name__ == "__main__":
    ...
    for frame_j in range(9):
        frame_i = frame_j
        score[frame_j], point_optimized[frame_j] = cma_optimizer(frame_j, env) #cma_optimizer(frame_i, env)

    dict_item = { 'Points': point_optimized,' score ' : score  }
    f = open("file_modified_handover_40_second_cma.pkl","wb")
