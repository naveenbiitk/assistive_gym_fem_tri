...
def iterating_across_poses(point, env, frame_i):
    ...
    dir_base = 'assistive-gym-fem/examples/optimal_frame_lying/'
...
def read_optimized_points(frame_i, env):

    # dict_item = {'score' : cost, 'point': point_optimized}
    with open('result_realtime_handover.pkl', 'rb') as f:
        loaded_dict = pickle.load(f)
    print(loaded_dict)
    points = np.array(loaded_dict['point']) # 192*3   
    cost = np.array(loaded_dict['score']) # 192
    return cost, points
...
if __name__ == "__main__":
    ...
    cost, point_optimized = read_optimized_points(frame_i, env)
