...
def calculate_obj_ik(env, target_pose_angles, target_position):
  ...
  weights_opt_ik = np.array([ 0.805,  1.1005,  0.001 ])
...
def cma_ws_optimizer(frame_i,env):
    ...
    while iteration < iteration_size:
        ...
        for _ in range(optimizer.population_size):
            ...
            writer.add_scalar('minimization_value', cost)
        ...
        print(f"fevals: {fevals}, Iteration: {iteration}, elapsed time: {elapsed_time:.2f}, mean reward = {mean:.2f}, min/max reward = {min:.2f}/{max:.2f}")
...
# torso_smpl+x = waist_point_simulation points
# x =  waist_point_simulation points - torso_smpl = 'frame_point' - body_joints_3d_est[0]
# add to every parts in smpl
...
if __name__ == "__main__":
    ...
    #env.create_sphere(radius=0.06, mass=0.0, pos=[-1,1,2], visual=True, collision=False, rgba=[1, 0, 0, 1]) #human hand
    ...
    target = env.create_sphere(radius=0.06, mass=0.0, pos=evaluation_point, visual=True, collision=False, rgba=[1, 0, 0, 1]) #human hand
    target.set_base_pos_orient(evaluation_point, task_orient)
    ...
    dict_item = {'score' : cost, 'point': point_optimized, 'task_point': evaluation_point, 'task_orient':task_orient, 'frame_point':frame_point, 'frame_orient': frame_orient, 'function_array':minimization_values ,'points_array':eval_point_values, 'simulated_human_positions':simulation_human_joint_positions }
    c_ = optimizing_across_poses(point_optimized,env,frame_i)
    f = open("result_realtime_handover.pkl","wb")
    pickle.dump(dict_item,f)
    
    # commenting out to save debug time sine i did not get to test
    # p = open("result_realtime_handover.json","w")
    # json.dump(dict_item, p)
