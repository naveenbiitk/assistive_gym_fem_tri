...
x_min = shoulder_pos[0]-(arm_length-0.1)
x_max = shoulder_pos[0]+(arm_length-0.1)

y_min = shoulder_pos[1]-arm_length
y_max = shoulder_pos[1]+0.0

z_min = shoulder_pos[2]-(arm_length-0.1)
z_max = shoulder_pos[2]+(arm_length-0.1)


x_ = np.linspace(x_min,x_max,8)
y_ = np.linspace(y_min,y_max,8)
z_ = np.linspace(z_min,z_max,6)
...
check_mask = np.zeros([len(points_sphere)])

i_ = len(points_sphere)
...
dict_item = { 'Points': points_sphere,'determinant' : det_list, 'jlwki' : jlwki_list, 'sum_force': sforce_list, 'sum_moment': smoment_list,'reach_check': check_mask,'fobj_moment':f_moment_list, 'fobj_angle':f_angle_list }
