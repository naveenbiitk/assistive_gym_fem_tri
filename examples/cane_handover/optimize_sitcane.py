...
def viewer(env_name):
    ...
    while sample_i<2:
        ...
        optimizing_across_poses(transform_pos, env)

        torso_pos, torso_orient = env.human.get_pos_orient(env.human.waist)
        generate_line(torso_pos, torso_orient, lineLen=0.45)

        time.sleep(9)
        sample_i += 1
    

def optimizing_across_poses(point, env):
        ...
        for h_j_ in range(-3,4,2):  #head_z
            for j_ in range(-3,4,2):   # waist orientation
                for i_ in range(-3,4,2):  # base orientation
                    ...
                    transform_pos = [point[0]+0.1,point[1],point[2]+0.5]
                    tool_pos = point #[point,point,point]
                    transform_orient = [0,0,0,1]
...
def generate_line(pos, orient, lineLen=0.15):
        ...
        p.addUserDebugLine(pos, toX, [1, 0, 0], 5)  #red
        p.addUserDebugLine(pos, toY, [0, 1, 0], 5)  #green
        p.addUserDebugLine(pos, toZ, [0, 0, 1], 5)  #blue
