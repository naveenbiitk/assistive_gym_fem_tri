...
def neck_angle(env_human, env_id, target_pos, disp=False):
    ...
    p0 = heye_pos
    p1 = generate_line_head(heye_pos, heye_orient, 0.5, disp)
    p2 = target_pos
...
def optimizing_function(points,orient, env_human, env_id, env):
    ...
    weights_opt = np.array([ 0.3,  0.3,  0.3 , 3, 2])
    ...
    #neck_angle(env_human, env_id, target_position, True)
    ...
    run_simulation(env_human, env_id, target_pose_angles , 12, env, target_position)
    env_human.set_joint_angles(env_human.right_arm_joints, target_pose_angles)
