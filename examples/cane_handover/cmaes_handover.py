...
def run_simulation(env_human, env_id, count_target, env, target_position):
    ...
    target_joint_angles_h = position_human_arm(env, target_position, attempts=70)
...
def optimizing_function(points, env_human, env_id, env):
    ...
    weights_opt = np.array([ 0.7,  0.5,  0.05 ])
    ...
    forces = np.array([env_human.get_force_torque_sensor(joints_j) for joints_j in env_human.right_arm_joints])
    ...
    linear_forces = np.sqrt( forces[:7,0]*forces[:7,0] + forces[:7,1]*forces[:7,1] + forces[:7,2]*forces[:7,2] )
    linear_moment = np.sqrt( forces[:7,3]*forces[:7,3] + forces[:7,4]*forces[:7,4] + forces[:7,5]*forces[:7,5] )
